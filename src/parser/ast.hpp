...
// Define the AST node types - to distinguish between the the different types of
// nodes in the AST class
...
public:
  ProgramNode(BlockNode *body) : AstNode(AstNodeType::Program), body(body) {}
...
public:
  TernaryOpNode(AstNode *condition, AstNode *left, AstNode *right)
      : AstNode(AstNodeType::TernaryOp), cond(condition), left(left),
        right(right) {}
...
public:
  NameNode(Token name) : AstNode(AstNodeType::Name), value(name) {}
...
public:
  BreakNode(Token keyword) : AstNode(AstNodeType::Break), kwd(keyword) {}
...
public:
  ContinueNode(Token keyword) : AstNode(AstNodeType::Continue), kwd(keyword) {}
...
public:
  PassNode(Token keyword) : AstNode(AstNodeType::Pass), kwd(keyword) {}
...
public:
  FunctionNode(Token fname, std::vector<AstNode *> params, AstNode *body)
      : AstNode(AstNodeType::Function), fname(fname), params(params),
        body(body) {}
...
public:
  IfNode(AstNode *cond, AstNode *trueBranch,
         ...
         AstNode *elseBranch)
      : AstNode(AstNodeType::If), cond(cond), trueBranch(trueBranch),
        elifBranches(elifBranches), elseBranch(elseBranch) {}
