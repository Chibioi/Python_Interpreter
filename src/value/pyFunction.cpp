...
PyFunction::PyFunction(
    ...
    size_t arity)
    : PyObject(ObjectType::Func), functor(functor),
      arity(arity) { // undefined scope for builtins
}
...
PyObject *PyFunction::call(std::vector<PyObject *> &args,
  ...
  if (this->functor) {
    ...
    if (expectedArgs) {
      ...
      for (size_t paramIdx = 0; paramIdx < expectedArgs; ++paramIdx) {
        const std::string &paramName =
            params[paramIdx]->unwrap_name_node()->get_lexeme();
    ...
    try {
      // Visit the body of the function, switching back to the interpreter's
      // context
